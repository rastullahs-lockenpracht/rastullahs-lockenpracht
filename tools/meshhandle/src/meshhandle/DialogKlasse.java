...
public class DialogKlasse {
    ...
    public void main() {
        ...
        File[] modulesdirs = (new File(files[0], "modules")
        ...
        moduleslist.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                ...
                File[] models = (new File((File) moduleslist.getSelectedItem(),
                        "models").listFiles(new FilenameFilter() {
                    public boolean accept(File f, String s) {
                ...
                modellist.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent event) {
                        ...
                        // Hier hängt sich das Teil bei geänderter Auswahl auf,
                        // anstatt, wie erhofft, das nicht mehr benötigte XML zu
                        // entfernen und ein neues anzulegen.
                        // Das könnte am noch bestehenden Zugriff des Converters
                        // liegen.
                        eventuellesxml = new File(files[1], ((File) modellist
                                .getSelectedItem()).getName()
                                + ".xml");
                        frame.getContentPane().add(modulPane,
                                BorderLayout.NORTH);
                        frame.getContentPane().add(modelsPane,
                                BorderLayout.WEST);
                        optionsPane.removeAll();
                        optionsPane = new MaterialsAendern((File) modellist
                                .getSelectedItem());
