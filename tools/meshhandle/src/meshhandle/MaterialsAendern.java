...
public class MaterialsAendern extends JPanel {
    ...
    protected MaterialsAendern(final File mesh) {
        ...
        ok.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (xmlneuschreiben(Float.parseFloat(factorfield.getText()),
                        model, newname.getText())) {
                    mesh.delete();
                    String[] args = {
    ...
    public boolean xmlneuschreiben(float factor, File model, String newname) {
            ...
            for (String s; (s = reader.readLine()) != null;) {
                if (s.indexOf("<submesh material=") != -1 && i < matname.length) {
                    dateiInhalt.append(s
                            .substring(0, SUBMESH_MATERIAL.length())
                            + matname[i]
                    ...
                    dateiInhalt.append(s.substring(0, pos[0] + 1) + newname
                            + ".skeleton" + s.substring(pos[1]) + "\n");
                } else {
                    dateiInhalt.append(s + "\n");
                }
            }
