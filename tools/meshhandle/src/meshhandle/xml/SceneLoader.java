...
public class SceneLoader extends XMLLoader {
    ...
    private static SceneNode processNode(Element sceneNodeElem) {
        ...
        scenenode.setPosition(Vector3.createFromXML(sceneNodeElem
                .getElementsByTagName("position").item(0)));
        scenenode.setScale(Vector3.createFromXML(sceneNodeElem
                .getElementsByTagName("scale").item(0)));
        scenenode.setRotation(Quaternion.createFromXML(sceneNodeElem
                .getElementsByTagName("rotation").item(0)));
        Element entityElem = (Element) sceneNodeElem.getElementsByTagName(
                "entity").item(0);
        scenenode.setEntity(new Entity(entityElem.getAttribute("name"),
                entityElem.getAttribute("meshFile")));
        return scenenode;
    }
}
