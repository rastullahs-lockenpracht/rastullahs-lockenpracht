...
public:
    ...
    {
        CPPUNIT_ASSERT_MESSAGE("Listener sollten fuer Gruppe 1 vorhanden sein",
            mCombat->mEventCasters[TEST_OBJ_GROUP].hasEventListeners());
        CPPUNIT_ASSERT_MESSAGE("Listener sollten fuer Gruppe 2 vorhanden sein",
            mCombat->mEventCasters[TEST_OPP_GROUP].hasEventListeners());
    ...
    void testInitializeKampfrunde()
    {
        CPPUNIT_ASSERT_EQUAL_MESSAGE(
            "Zeit fuer eine Aktion is 1/4 KR", 0.25f * (float)Date::ONE_KAMPFRUNDE, mCombat->mTimeOfAction);
        CPPUNIT_ASSERT_MESSAGE("Creature 1 sollte INI > 8 haben", mCombat->getParticipant(mCreature1)->initiative > 8);
        CPPUNIT_ASSERT_MESSAGE(
            "Creature 2 sollte 1 < INI < 8 haben", mCombat->getParticipant(mCreature2)->initiative > 1);

        CPPUNIT_ASSERT_MESSAGE(
            "INI sollte nicht mehr auf Start stehen", mCombat->mCurrentInitiative != mCombat->INI_START);
        ...
        CPPUNIT_ASSERT_MESSAGE(
            "Kein Event sollte bei Gruppe 2 empfangen worden sein", mCombatTestResultGroup2->getLastEvent() == NULL);

        CPPUNIT_ASSERT_MESSAGE(
            "Ein Event sollte bei Gruppe 1 empfangen worden sein", mCombatTestResultGroup1->getLastEvent() != NULL);
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Event sollte AskForAction sein", CombatEvent::CEV_ASK_FOR_ACTION,
            mCombatTestResultGroup1->getLastEvent()->getEventType());
        CPPUNIT_ASSERT_MESSAGE("SlowMo sollte aktiviert sein",
            mCombatTestResultGroup1->getLastEvent()->isGroupRelevant(TEST_OBJ_GROUP) == true);
        CPPUNIT_ASSERT_EQUAL_MESSAGE("Creature 1 sollte das Event erhalten haben", CombatEvent::CEV_ASK_FOR_ACTION,
            mCombatTestResultGroup1->getLastEvent()->getEventType());
    }
    ...
    void testCanDefend()
    {
        ...
        CPPUNIT_ASSERT_MESSAGE(
            "Creature1 kann sich gegen Creature2 verteidigen", mCombat->canDefend(part2, part1) == true);

        part1->pareesLeft = 0;
        part2->pareesLeft = 0;

        CPPUNIT_ASSERT_MESSAGE(
            "Creature2 kann sich nicht mehr gegen Creature1 verteidigen", mCombat->canDefend(part1, part2) == false);
        CPPUNIT_ASSERT_MESSAGE(
            "Creature1 kann sich nicht mehr gegen Creature2 verteidigen", mCombat->canDefend(part2, part1) == false);
    }

    void testProcessAttack()
    {
        mCombat->setNextAction(mCreature1, new CombatActionAttack(mCreature1, mCreature2, "Raufen"),
            mCombatTestResultGroup1->getLastEvent()->getId());
        ...
        Combat::Participant* part2 = mCombat->getParticipant(mCreature2);
        CPPUNIT_ASSERT_MESSAGE(
            "Creature2 kann sich gegen Creature1 verteidigen", mCombat->canDefend(part1, part2) == true);
        ...
        CPPUNIT_ASSERT_MESSAGE(
            "Kein Event sollte bei Gruppe 1 empfangen worden sein", mCombatTestResultGroup1->getLastEvent() == NULL);
        CPPUNIT_ASSERT_MESSAGE(
            " Ein Event sollte bei Gruppe 2 empfangen worden sein", mCombatTestResultGroup2->getLastEvent() != NULL);
    }
    ...
    void testAutoEndeEreignis()
    {
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE(
            "Action des Ereignisses ist Attacke", CombatAction::ATTACK, mCombat->getLastEvent()->action->getType());
    ...
    void setUp()
    {
        ...
        mCombat = new Combat();
        mCombat->add(mCreature1, TEST_OBJ_GROUP);
        mCombatTestResultGroup1 = new CombatTestResult(TEST_OBJ_GROUP);
        mCombat->add(mCreature2, TEST_OPP_GROUP);
        mCombatTestResultGroup2 = new CombatTestResult(TEST_OPP_GROUP);
