...
namespace rl
{
    ...
    MeshAnimation* CreatureController::setAnimation(const Ogre::String& name, Ogre::Real speed,
        unsigned int timesToPlay, const Ogre::String& collisionName, Real weight)
    {
    ...
    {
        return mCreature->getActor()->getPhysicalThing()->getOrientation().Inverse()
    ...
    bool CreatureController::run(Real elapsedTime)
    {
        ...
        if (getAbstractLocation() == AL_AIRBORNE)
        {
            ...
            if (timeSinceLastFloorContact < Time(Date::ONE_SECOND) * 0.19 && abs(speed.y) < 0.6)
            {
        ...
        else
        {
                ...
                abs(speed.y) > 0.6)
            {
                ...
                // oss << "Raycast for floor: speed.y: " << speed.y << "      timeSinceLastFloorContact: " <<
                // timeSinceLastFloorContact;

                // find the distance to the floor:
                // raycasts
                PhysicsMaterialRaycast::MaterialVector materialVector;
                materialVector.push_back(
                    PhysicsManager::getSingleton().getMaterialID("camera")); // should we perhaps only use level here?
                ...
                RaycastInfo info = mRaycast.execute(
                    PhysicsManager::getSingleton()._getNewtonWorld(), &materialVector, start, end, false);
    ...
    void CreatureController::userProcess(OgreNewt::ContactJoint& contactJoint, Real timestep, int threadid)
    {
        for (OgreNewt::Contact contact = contactJoint.getFirstContact(); contact; contact = contact.getNext())
        {
            ...
            /*
            //        setContactNormalDirection(((Vector3::UNIT_Y.dotProduct(point-charPos)*Vector3::UNIT_Y + charPos) -
            point).normalisedCopy()); setContactNormalDirection(point - (charPos + charHeight/2));
            ...
            /*
                    if( stepHeight < 0.4 )
                    {
                        if(stepHeight > 0.01f) // experimantal value,
                                               ...
                                               // too high means the creature stops if moving slowly onto a step because
               of the friction
                        {
