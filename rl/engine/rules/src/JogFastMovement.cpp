...
namespace rl
{
    JogFastMovement::JogFastMovement(CreatureController* creature)
    ...
    bool JogFastMovement::isPossible() const
    {
        if (mMovingCreature->getMovement() == this)
            return WalkMovement::isPossible() && mMovingCreature->getCreature()->getAu() > 6;
        else
            return WalkMovement::isPossible() && mMovingCreature->getCreature()->getAu() > 8;
    }
    ...
    void JogFastMovement::setAnimation(Ogre::Real elapsedTime)
    {
        ...
        MeshAnimation* meshAnim = mMovingCreature->setAnimation(
            mAnim.first, -mMovingCreature->getVelocity().z * mAnim.second, 0, mAnim.first, weight);
