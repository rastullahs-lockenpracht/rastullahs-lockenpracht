...
namespace rl
{
    struct InitiativeComparator : std::binary_function<std::pair<int, Combatant*>, std::pair<int, Combatant*>, bool>
    {
        /// \todo take into account other values of the combatant according to TDE rules,
        /// in case initiative is equal.
        bool operator()(const std::pair<int, Combatant*>& c1, const std::pair<int, Combatant*>& c2) const
        {
    ...
    void Combat::addOpponent(Combatant* combatant)
    {
        ...
        GameEventLog::getSingleton().logEvent(combatant->getName() + " wurde in einen Kampf verwickelt.", GET_COMBAT);
    ...
    void Combat::removeOpponent(Combatant* combatant)
    {
        ...
        GameEventLog::getSingleton().logEvent(combatant->getName() + " nimmt nicht mehr am Kampf teil.", GET_COMBAT);
    ...
    void Combat::checkAndMarkCombatant(Combatant* combatant)
    {
        ...
        for (CombatantActionsMap::iterator it = mCombatantActions.begin(); it != mCombatantActions.end(); ++it)
        {
    ...
    {
        for (CombatantSet::iterator i = mRemovedCombatants.begin(); i != mRemovedCombatants.end(); ++i)
        {
            // Are we owner of this combatant? If so, delete it.
            CombatantSet::iterator it = std::find(mOwnedCombatants.begin(), mOwnedCombatants.end(), *i);
    ...
    void Combat::start()
    {
        ...
        for (std::vector<Combatant*>::const_iterator it = combatants.begin(); it != combatants.end(); ++it)
        {
            mCombatantQueue.push_back(
                std::make_pair((*it)->getCreatureController()->getCreature()->doInitiativeWurf(), *it));
        }
    ...
    bool Combat::canAttack(Combatant* actor, Combatant* target) const
    {
        ...
        Real distance = MathUtil::distance(
            target->getCreature()->getWorldBoundingBox(), actor->getCreature()->getWorldBoundingBox());
    ...
    void Combat::beginRound()
    {
        ...
        GameEventLog::getSingleton().logEvent(
            "Runde " + CeGuiString(StringConverter::toString(mCurrentRound)), GET_COMBAT);
        // Show ini for this round.
        for (CombatantQueue::iterator it = mCombatantQueue.begin(); it != mCombatantQueue.end(); ++it)
        {
    ...
    void Combat::executeRound()
    {
        ...
        for (size_t actionIndex = 0; actionIndex < 3; ++actionIndex)
        {
            for (CombatantQueue::iterator it = mCombatantQueue.begin(); it != mCombatantQueue.end(); ++it)
            {
                ...
                JobSet* jobSetOutcome
                    = new JobSet(); // damage is applied after combat animations to prevent premature reactions
                    ...
                    if (actionIndex < actionsIt->second.size())
                    {
                        ...
                        else if (entry.aktion == BEWEGEN)
                        {
                            GameEventLog::getSingleton().logEvent(combatant->getName() + " l‰uft nach "
                                    + CeGuiString(StringConverter::toString(entry.targetPos)),
                                GET_COMBAT);
                        ...
                        {
                            GameEventLog::getSingleton().logEvent(
                                combatant->getName() + " l‰uft zu " + entry.target->getName(), GET_COMBAT);
        ...
        mAnimationSequenceTicket = JobScheduler::getSingleton().addJob(
            jobQueue, JobScheduler::JP_NORMAL, 0.0f, Ogre::Math::POS_INFINITY, this);
    }
    ...
    void Combat::doAttacke(JobSet* jobSetAnims, JobSet* jobSetDamage, Combatant* actor, Combatant* target)
    {
        ...
        GameEventLog::getSingleton().logEvent(actor->getName() + " attackiert " + target->getName(), GET_COMBAT);
        ...
        if (aresult >= RESULT_ERFOLG)
        {
            ...
            if (it != mCombatantReactions.end())
            {
                ...
                if (it->second == PARADE)
                {
                    ...
                    {
                        GameEventLog::getSingleton().logEvent("Erfolg, nicht pariert, Treffer!", GET_COMBAT);
                    }
    ...
    void Combat::doDamage(JobSet* jobSet, DamageStrength damageStrength, Combatant* actor, Combatant* target)
    {
        if (damageStrength != DMG_NONE) ///@todo half/double damage
        {
            ...
            CeGuiString msg
                = actor->getName() + " trifft fuer " + CeGuiString(StringConverter::toString(tp)) + " Trefferpunkte";
    ...
    bool Combat::onGameObjectLifeStateChanged(
        ...
        if (newstate & Effect::LS_NO_COMBAT)
        {
            ...
            for (size_t i = 0; i < mCombatantQueue.size(); ++i)
            {
                ...
                if (gameobject == curCreature)
                {
                    ...
                    CombatantSet::iterator it = std::find_if(
                        mOpponents.begin(), mOpponents.end(), std::bind2nd(FindCombatantByCreature(), curCreature));
                    ...
                    {
                        CombatantSet::iterator it = std::find_if(
                            mAllies.begin(), mAllies.end(), std::bind2nd(FindCombatantByCreature(), curCreature));
                        if (it != mAllies.end())
                        {
