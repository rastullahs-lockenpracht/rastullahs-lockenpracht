...
namespace rl
{
    ...
    void Combatant::doAttacke(JobSet* jobSet, Combatant* target, int attackeResult, bool parade, int paradeResult)
    {
    ...
    {
        Job* job = new GoToJob(getCreature(), target->getCreature(), getActiveWeapon()->getMaximumDistance(), 3.0f);
        jobSet->add(job);
    }
