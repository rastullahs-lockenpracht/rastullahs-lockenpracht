...
namespace rl
{
    ...
    const CeGuiString Quest::STATE_NAMES[5] = { "OPEN", "ASSIGNED", "FAILED", "SUCCEEDED", "COMPLETED" };

    const CeGuiString Quest::KNOWN_NAMES[2] = { "UNKNOWN", "KNOWN" };
    ...
    int Quest::getPartsDone() const
    {
        ...
        int done = 0;
        for (QuestVector::const_iterator it = mSubquests.begin(); it != mSubquests.end(); it++)
        {
            Quest* cur = (*it);
            if (cur->getState() == Quest::COMPLETED || cur->getState() == Quest::SUCCEEDED)
                done++;
        }
        return done;
    }
    ...
    void Quest::setState(Quest::State state)
    {
        if (mState != state)
        {
            ...
            if (state == Quest::OPEN && mParent != NULL && mParent->getState() == Quest::UNKNOWN)
            {
    ...
    {
        if (getPartsDone() >= getPartsToDo() && mState != Quest::COMPLETED && mState != Quest::SUCCEEDED)
        {
    ...
    bool Quest::getKnownFromName(const CeGuiString knownName)
    {
        ...
        Throw(IllegalArgumentException, (knownName + " is no valid quest known state.").c_str());
    }
