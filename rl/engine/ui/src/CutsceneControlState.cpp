...
namespace rl
{
    ...
    CutsceneControlState::CutsceneControlState(CommandMapper* cmdMapper, Actor* camera)
        : ControlState(cmdMapper, camera, NULL, CST_CUTSCENE)
        , mTarget(NULL)
    {
    }
    ...
    {
        dynamic_cast<Camera*>(mCameraActor->getControlledObject()->getMovableObject())->lookAt(point);
    }
    ...
    CameraPosition CutsceneControlState::getBestCameraPosition() const
    {
        ...
        for (std::vector<CameraPosition>::const_iterator it = mPositions.begin(), end = mPositions.end(); it != end;
             ++it)
        {
        }
    ...
    void CutsceneControlState::addCamera(
        const Vector3& position, const Quaternion& orientation1, const Quaternion& orientation2)
    {
