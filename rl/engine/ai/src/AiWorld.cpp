...
namespace rl
{
        ...
        : mObstacles()
    {
        ...
        mProximityCheck
            = new OpenSteer::LQProximityDatabase<OpenSteer::AbstractVehicle*>(center, dimensions, divisions);
        ...
        const AbstractVehicle& vehicle, PathIntersection& pi) const
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            info = materialRaycast.execute(mNewtonWorld, &mMaterialsToConsider, castPos, castFutPos);
        ...
        default:
            ...
            /*        pi.surfaceNormal.normalisedCopy();
                    // jedenfalls gilt dafuer, dass der abstand zwischen surfacePoint+surfaceNormal und _pos minimal
               sein muss if( ((pi.surfacePoint + pi.surfaceNormal) - _pos).length() >
