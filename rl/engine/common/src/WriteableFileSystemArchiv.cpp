...
namespace rl
{
    ...
    WriteableFileSystemArchive::WriteableFileSystemArchive(const Ogre::String& name, const Ogre::String& archType)
        : Ogre::FileSystemArchive(name, archType)
    {
    }
    ...
    DataStreamPtr WriteableFileSystemArchive::open(const String& filename) const
    {
        ...
        if (origStream->fail())
        {
            delete origStream;
            OGRE_EXCEPT(
                Exception::ERR_FILE_NOT_FOUND, "Cannot open file: " + filename, "WriteableFileSystemArchive::open");
        }

        /// Construct return stream, tell it to delete on destroy
        WriteableFileStreamDataStream* stream
            = new WriteableFileStreamDataStream(filename, origStream, tagStat.st_size, true);
