...
namespace rl
{
    ...
    void Fmod4SoundStitching::load() throw(RuntimeException)
    {
        ...
        FMOD_RESULT res
            = mDriver->_getFmodSystem()->createStream(getSoundResource()->getName().c_str(), mode, &exinfo, &mSound);
    ...
    void Fmod4SoundStitching::putSoundIntoSlot(unsigned int slot, CeGuiString label)
    {
        if (isValid())
        {
            ...
            FMOD::Sound* sound;
            FMOD_RESULT result
                = mDriver->_getFmodSystem()->createStream(mSoundCache[label]->getName().c_str(), mode, NULL, &sound);
    ...
    unsigned int Fmod4SoundStitching::getPlayingSlot()
    {
        if (isValid())
        {
            ...
            FMOD_RESULT result = mChannel->getPosition(
                &currentslot, (FMOD_TIMEUNIT)(FMOD_TIMEUNIT_SENTENCE_SUBSOUND | FMOD_TIMEUNIT_BUFFERED));
            CHECK_FMOD4_ERRORS(result);
            return currentslot;
        }
        return -1;
