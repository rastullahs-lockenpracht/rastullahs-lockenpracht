...
namespace rl
{
    ...
    void GenericWorld::setCastShadows(bool enabled)
    {
        ...
        if (enabled)
        {
            Ogre::String tmp = ConfigurationManager::getSingleton().getStringSetting("Video", "Cast Shadows");
        ...
        if (castShadows)
        {
            int textureSize = ConfigurationManager::getSingleton().getIntSetting("Video", "Shadow Texture Size");
