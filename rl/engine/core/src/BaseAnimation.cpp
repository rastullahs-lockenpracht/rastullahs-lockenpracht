...
namespace rl
{
    ...
    BaseAnimation::BaseAnimation(Ogre::Real length, Ogre::Real speed, unsigned int timesToPlay, bool paused)
        : mLength(length)
        , mPaused(paused)
    ...
    void BaseAnimation::addTime(Ogre::Real timePassed)
    {
        if (!mPaused)
        {
            ...
            if (fabs(timePassed) - 0.0001 > 0)
            {
                ...
                MessagePump::getSingleton().sendMessage<MessageType_AnimationFrameReached>(this, elapsedTime);
            }
