...
namespace rl
{
    ...
    Zone::Zone(long id, bool needsToBeSaved)
        : mId(id)
        , mEaxPreset("")
    ...
    void Zone::removeLight(Actor* light)
    {
        ...
        std::list<Actor*>::iterator it = std::find(mLights.begin(), mLights.end(), light);
        if (it == mLights.end())
    ...
    {
        std::list<Ogre::String>::iterator it = std::find(mSounds.begin(), mSounds.end(), sound);
        if (it == mSounds.end())
    ...
    void Zone::removeTrigger(Trigger* trigger)
    {
        ...
        std::list<Trigger*>::iterator it = std::find(mTriggers.begin(), mTriggers.end(), trigger);
        if (it == mTriggers.end())
