...
namespace rl
{
    ...
    public:
         ...
         * @param nocache if set to true a new collision will be created (without looking for an existing one in the
         * cache); the new collision is not added to the cache
         */
        OgreNewt::CollisionPtr createCollisionFromEntity(Ogre::Entity* entity, const GeometryType& geomType,
         ...
         * // no scaling at the moment, the correct scale must be given (OgreNewt cannot scale a collision, this is a
         * limitation
         * // of newton. It is only possible to 'scale' a convex collision by created a ConvexHullModifier (a new
         * collision,
         * // that uses the old one and applies transformation through a matrix), so for aabbs (-> convex collisions) a
         * scale
         * // that udpates with the scale of the node could be implemented, but as this would be different for
         * treecollisions,
         ...
         * @param geomType specifies the type of collision primitiv to create.
         * @param offset gives the offset of the coordinate system of the coll. primitiv, it is relative to the standard
         * offset (center of the aabb)
         * @param orientation Quaternion gives an euler rotation for the coordinate system of the coll. primitiv
         * @param mass gives the mass of the collision primitive used for calculating the inertia
         * @param inertia Vector3 returns the inertia coefficients for the created collision primitiv
         * @param centerOfMass Vector3 returns the centerOfMass coefficients for the created collision primitiv
         ...
         * cache); the new collision is not added to the cache
         */
        OgreNewt::CollisionPtr createCollisionFromAABB(const Ogre::String& name, const Ogre::AxisAlignedBox& aabb,
    ...
    protected:
         ...
         */
        OgreNewt::ConvexCollisionPtr createBox(
         ...
         * @param orientation when not null, specifies the euler angle of orientation.
         */
        OgreNewt::ConvexCollisionPtr createPyramid(
            const Ogre::AxisAlignedBox& aabb, const Ogre::Vector3& offset, const Ogre::Quaternion& orientation);
         ...
         * @param orientation when not null, specifies the euler angle of orientation.
         */
        OgreNewt::ConvexCollisionPtr createSphere(
            const Ogre::AxisAlignedBox& aabb, const Ogre::Vector3& offset, const Ogre::Quaternion& orientation);
         ...
         * @param orientation when not null, specifies the euler angle of orientation.
         */
        OgreNewt::ConvexCollisionPtr createEllipsoid(
            const Ogre::AxisAlignedBox& aabb, const Ogre::Vector3& offset, const Ogre::Quaternion& orientation);
         ...
         * @param orientation when not null, specifies the euler angle of orientation.
         */
        OgreNewt::ConvexCollisionPtr createCapsule(
            const Ogre::AxisAlignedBox& aabb, const Ogre::Vector3& offset, const Ogre::Quaternion& orientation);
        ...
        //! class for caching convex collisions (box...convexhull), saves additionally the scale, so it can be rescaled
        //! if necessary
