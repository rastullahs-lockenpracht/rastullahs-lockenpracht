...
void MeshActor::attachActorToBone(Actor* actor, String name)
{
    if( mEntity->hasSkeleton() )
    {
        ...
        {
            CoreSubsystem::log( String( TYPENAME ) + " - " + this->getName() + " : Befestigen am Knochen mit dem Name " + name + " gescheitert. Kein Ogre-Movable gefunden." );
        }
    }
    else
    {
        CoreSubsystem::log( String( TYPENAME ) + " - " + this->getName() + " : Befestigen am Knochen mit dem Namen " + name + " gescheitert. Kein Skelett vorhanden" );
    }
}
...
void MeshActor::attachActorToBone(Actor* actor, unsigned short id)
{
    if( mEntity->hasSkeleton() )
    {
        if( actor->getSceneNode()->getAttachedObject(0) != 0 )
        {
            ...
            {
				CoreSubsystem::log( String( TYPENAME ) + " - " + this->getName() + " : Befestigen am Knochen mit der Id " + StringConverter::toString(id) + " gescheitert. Bone nicht gefunden." );
            }
        }
        else
        {
            CoreSubsystem::log( String( TYPENAME ) + " - " + this->getName() + " : Befestigen am Knochen mit der Id " + StringConverter::toString(id) + " gescheitert. Kein Ogre-Movable gefunden." );
        }
    }
    else
    {
        CoreSubsystem::log( String( TYPENAME ) + " - " + this->getName() + " : Befestigen am Knochen mit der Id " + StringConverter::toString(id) + " gescheitert. Kein Skelett vorhanden." );
    }
}
...
void MeshActor::detachActorFromBone(Actor* actor )
{
    if( mEntity->hasSkeleton() )
    {
        if( actor->getSceneNode()->getAttachedObject(0) != 0 )
        {
            try
            {
                mEntity->detachObjectFromBone( actor->getSceneNode()->getAttachedObject(0)->getName() );
