...
namespace rl {
    ...
    Weapon* ItemDataLoader::processWeapon(int group, DOMElement* weaponXml)
    {
		...
		// Containerplatz f�r andere Gegenst�nde, die dieser aufnahmen kann
	...
	Armor* ItemDataLoader::processArmor(DOMElement* armorXml)
	{
		...
		// R�stungsschutz f�r bestimmte Zonen
		...
		// Neuen R�stungsprototyp erzeugen und zur�ckgeben
		Armor* a = new Armor(
			name,
			desc
			);
	...
	Item* ItemDataLoader::processItem(DOMElement* itemXml)
	{
		...
		// Neuen Itemprototyp erzeugen und zur�ckgeben
		Item* i = new Item(
			name,
			desc
			);
